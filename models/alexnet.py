...
__all__ = [ 'AlexNet', 'alexnet', 'alexnetc']
...
class AlexNetCIFAR(nn.Module):
    def __init__(self, features, num_classes, sobel):
        ...
        self.classifier = nn.Sequential(nn.Dropout(0.5),
                            ...
                            nn.Linear(410, 410),
                            nn.ReLU(inplace=True))
        self.top_layer = nn.Linear(410, num_classes)
        self._initialize_weights()
...
def alexnetc(sobel=False, bn=True, out=1000):
    dim = 2 + int(not sobel)
    model = AlexNetCIFAR(make_layers_features(CFG_CIFAR, dim, bn=bn), out, sobel)
    return model